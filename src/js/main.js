...
const createMenuesktop = () => {
  const menu = $("#cabecalho .nivel-um");
  const menuItem = $("#cabecalho .nivel-um > li");
  const menuSize = $("#cabecalho .nivel-um > li").length;
  const moreCat = `
    <li class="categoria-marcas com-filho borda-principal">
      ...
      <ul class="nivel-dois borda-alpha colunas-1 more-cat"></ul>
    </li>
  `;

  if (pageW >= mobResMax && menuSize > 4) {
    menu.append(moreCat);
    menuItem.slice(4).appendTo(".more-cat");
  }
};
...
const createHeaderMobile = () => {
  //New Html Tags
  const BtMobMenu = $(
    '<li class="mobMenu"><i class="fas fa-bars"></i><i class="fas fa-times"></i></li>'
  );
  const mobSearch = $(
    '<li class="mobSearch"><i class="fas fa-search-plus"></i><i class="fas fa-search-minus"></i></li>'
  );
  ...
  fetch(videosUrl)
    ...
    .then(function (data) {
      ...
      const formatTitle = (givenTitle) => {
        ...
        const count = 55;
        const result =
          strTitle.slice(0, count) + (strTitle.length > count ? "..." : "");
        return result;
      };
        ...
        <div class="content">
          ...
          <div class="video main">
            <a href="https://www.youtube.com/watch?v=${
              ...
              style="background-image: url(https://i.ytimg.com/vi_webp/${
                data.items[0].id.videoId
              }/maxresdefault.webp)"></a>
            <h2 class="video_title">${formatTitle(
              data.items[0].snippet.title
            )}</h2>
            <div class="desc">por <strong>Toni Armas</strong> - ${setFormattedDate(
          ...
          <div class="side">
            ...
            <div class="video">
              ...
              <div class="desc">por <strong>Toni Armas</strong> - ${setFormattedDate(
                data.items[1].snippet.publishedAt
              )}</div>
            </div>
            ...
            <div class="video">
              ...
              <h2 class="video_title">${formatTitle(
                data.items[2].snippet.title
              )}</h2>
              <div class="desc">por <strong>Toni Armas</strong> - ${setFormattedDate(
                data.items[2].snippet.publishedAt
              )}</div>
            </div>
...
const createProductDetail = () => {
  ...
  relatedProdsTitle
    .addClass("titulo-categoria")
    .html("<strong>Produtos relacionados</strong>");
};
